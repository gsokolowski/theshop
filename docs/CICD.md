...
An older version of a prior doc described **`develop`**, **`stage`**, and deploy-on-push to staging. The **current** workflow is **`main` + `v*`** only. You can reintroduce staging by adding `on.push.branches: [stage]` and a second deploy target, or a separate workflow, and update **this** document when that exists.
