<!-- dispalys all categories list in a sidebar -->
<template>
    <h6 class="mt-3 mb-3">Categories</h6>
    <ul class="nav d-flex flex-column">
        <li class="nav-item mb-1" v-for="category in productsStore.categories"
            :key="category.id">
            <!-- ✅ CHANGED: highlight active category when combined filters include this slug -->
            <button
                class="btn btn-sm btn-link"
                :class="productsStore.filters.categorySlug === category.slug ? 'text-primary fw-semibold' : 'text-dark'"
                @click="productsStore.filterProductsByCategory(category.slug)"
                >
                    {{ category.name }}
            </button>
        </li>
    </ul>
</template>

<script setup>
    // import the useProductsStore
    import { useProductsStore } from '../../stores/useProductsStore.js'

    // define the store variable and import the useProductsStore
    const productsStore = useProductsStore()
</script>

<style scoped>

</style>